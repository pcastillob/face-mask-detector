...
def enviar_datos(temp,mascarilla):
#    global sql
    try:            
        ...
        mycursor.callproc('add_datos',[2,datetime.datetime.now(),temp,mascarilla,])
        mydb.commit()    
    except mysql.connector.Error as error:
        print("falló la ejecución{}".format(error))
    finally:
            ...
            for (box, pred) in zip(locs, preds):
                ...
                if  contador >= 5:
                    ...
                    if not temp_bool:
                        ...
                        temp = float(temp)
                        if temp > 35 and temp < 41:
                            pausa_event.set()
                            #modificar acá para poner el codigo del sensor
                            mask_bool = True if totalMask > totalSinMask else False
                            temp_aceptable = True if 37.4 > temp else False
                            totalMask = 0
                            totalSinMask = 0   
                            segundos = 5
                            resultado_texto =""   

                            if mask_bool and temp_aceptable:
                                resultado_texto = "pasa"
                                audio_aprobado.play()
                            elif mask_bool and not temp_aceptable:
                                audio_tempElevada.play()
                                resultado_texto = "elevada"           
                                segundos=17                     
                            elif not mask_bool and not temp_aceptable:
                                audio_noCumpleReq.play()
                                resultado_texto = "elevada"
                                segundos=17
                            else:
                                audio_pongaseMask.play()
                                resultado_texto = "denegado"
                            enviar_datos(temp,mask_bool)
                            resultado_queue.put_nowait(resultado_texto)
                            time.sleep(segundos)
                            pausa_event.clear()
