...
#src=0 indica la cámara 0, puede haber más en el equipo y se iran enumerando con 1 , 2, etc.
...
#    global sql
    try:            
        ...
        mycursor.callproc('add_datos',[2,datetime.datetime.now(),temp,])
        mydb.commit()    
        ...
        try:
            ...
            if self.resultado_evento.is_set():
                ...
                if res =="pasa" or res=="denegado":
                    ...
                    self.label.configure(image=self.imagen,text="\n\n\n\n\n\n\n\n\n\n\n"+hora,fg="white",font=self.font)
                    self.label.image=self.imagen
                    self.window.after(6000,self.update)
                else:
